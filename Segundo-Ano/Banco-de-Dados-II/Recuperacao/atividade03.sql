...
SELECT cli.nome_cliente, f.nome_funcionario, p.nome_produto, v.quantidade_produto
FROM venda v
INNER JOIN cliente cli ON cli.id_cliente = v.cliente_id
INNER JOIN cidade cid ON cid.id_cidade = cli.cidade_id
INNER JOIN funcionario f ON f.id_funcionario = v.funcionario_id
INNER JOIN produto p ON p.id_produto = v.produto_id
WHERE cid.nome_cidade = 'Paranavai'
GROUP BY cli.nome_cliente, f.nome_funcionario, p.nome_produto
ORDER BY v.quantidade_produto DESC